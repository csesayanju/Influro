...
| Table | Purpose |
|-------|--------|
| **brands** | One per user (onboarding); RLS by `user_id`. Columns: id, user_id, name, website, category, created_at, updated_at |
...
| **influencers** | Per campaign. Columns: id, campaign_id, handle, platform, follower_count, agreed_fee, platform_data, created_at, updated_at |
...
| **click_events** | Each UTM hit; utm_link_id nullable for untracked-organic. Columns: id, utm_link_id, session_id, ip_hash, user_agent, country, source, created_at |
| **conversions** | Razorpay webhook; payment_id UNIQUE. Columns: id, utm_link_id, influencer_id, payment_id, amount_paise, converted_at, created_at |
| **fraud_scores** | Bot score 0–1 per influencer; UNIQUE(influencer_id). Columns: id, influencer_id, score, refreshed_at, created_at |
